...
* single work directory containing output, org, backup that can be ignore by vc
* unescaped filenames everywhere (potentially except source)
* version specification

Open questions:
* Layout
  * config files in main directory, with a .coffle directory? (must be escaped)
  * config files in a source subdirectory?
...
'-.coffle
  |-coffle.yaml
  |-host_classes.yaml/rb
  |-...
  '-work    <---- ignore for version control
    |-output -> unescaped
	|-org    -> unescaped
	'-backup -> unescaped
