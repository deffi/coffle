...
.target_exist? stinkt
* .exist durch .present ersetzen, wo angemessen
* .file/.directory durch .proper_ ersetzen
* test: target already exists and is a link => not recognized as existing in backup dir?
* we should rename .directory?, .file?, and potentially .exist? to avoid
  mistakes


On directories:
* Can we uninstall a directory?
* Can we have directory entries at all? (nb: empty directories!)
...
* Should there be a single .coffle directory in source?
  ...
  => are coffle source directories marked?
...
*         |file/dir  |present |*         ||            refuse           |target was replaced
----------+----------+--------+----------++--------------+--------------+-------------------
