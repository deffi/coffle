...
Compatibility:
* Not using any gems because gems may not be available
* Not using lazy_attr_reader because 1.8.5 has no instance_variable_defined?
...
.
|-_config file templates   <---- escaped file names
|-.coffle_source.yaml
'-.coffle
  |-coffle files
  |-...
  '-work                   <---- ignore for version control
    |-output -> unescaped  <---- links point here
	|-org    -> unescaped
	'-backup -> unescaped

We could also keep the source files in a subdirectory:
.
|-[.]coffle_source.yaml
|-source
