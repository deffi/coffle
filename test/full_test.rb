	...
	class EntryTest <Test::Unit::TestCase
		...
		def test_full
			with_testdir do |dir|
				...
				##### Created the expected_install directory

				# Source - same as actual
				FileUtils.cp_r actual.join("source").to_s, expected_install.join("source").to_s

				# Output
				expected_install.join("source", ".output").mkpath
				expected_install.join("source", ".output", "_reg_file"            ).touch
				expected_install.join("source", ".output", "_reg_dir"             ).mkdir
				expected_install.join("source", ".output", "_reg_dir", "reg_file" )  .touch
				expected_install.join("source", ".output", "_ex_file"             ).touch
				expected_install.join("source", ".output", "_ex_dir"              ).mkdir
				expected_install.join("source", ".output", "_ex_dir", "ex_file"   )  .touch
				expected_install.join("source", ".output", "_link_dir"            ).mkdir
				expected_install.join("source", ".output", "_link_dir", "ex_file" )  .touch

				# Org
				expected_install.join("source", ".output", ".org").mkpath
				expected_install.join("source", ".output", ".org", "_reg_file"            ).touch
				expected_install.join("source", ".output", ".org", "_reg_dir"             ).mkdir
				expected_install.join("source", ".output", ".org", "_reg_dir", "reg_file" )  .touch
				expected_install.join("source", ".output", ".org", "_ex_file"             ).touch
				expected_install.join("source", ".output", ".org", "_ex_dir"              ).mkdir
				expected_install.join("source", ".output", ".org", "_ex_dir", "ex_file"   )  .touch
				expected_install.join("source", ".output", ".org", "_link_dir"            ).mkdir
				expected_install.join("source", ".output", ".org", "_link_dir", "ex_file" )  .touch

				# Backup
				expected_install.join("source", ".backup").mkpath
				expected_install.join("source", ".backup", ".ex_file"             ).touch
				expected_install.join("source", ".backup", ".ex_dir"              ).mkdir
				expected_install.join("source", ".backup", ".ex_dir", "ex_file"   )  .touch
				expected_install.join("source", ".backup", ".link_dir"            ).mkdir
				expected_install.join("source", ".backup", ".link_dir", "ex_file" )  .touch
 
				# Target
				expected_install.join("target").mkpath
				expected_install.join("target", ".reg_file"                     ).make_symlink("../source/.output/_reg_file")
				expected_install.join("target", ".reg_dir"                      ).mkdir
				expected_install.join("target", ".reg_dir", "reg_file"          )  .make_symlink("../../source/.output/_reg_dir/reg_file")
				expected_install.join("target", ".ex_file"                      ).make_symlink("../source/.output/_ex_file")
				expected_install.join("target", ".ex_dir"                       ).mkdir
				expected_install.join("target", ".ex_dir", "ex_file"            )  .make_symlink("../../source/.output/_ex_dir/ex_file")
				...
				expected_install.join("target", ".link_dir_target", "ex_file"   )  .make_symlink("../../source/.output/_link_dir/ex_file")
				...
				expected_uninstall.join("source", ".backup").rmtree
				expected_uninstall.join("source", ".backup").mkpath
